...
| `type` | 12 | 2 | `CLUSTERMSG_TYPE_*` (0–10). Selects how `data` is interpreted; see [§1.3](#13-data-by-message-type). |
...
In the **header**, `flags` describe the **sender** (normally `MASTER` or `SLAVE`, plus capability bits). `PFAIL` / `FAIL` matter most in **gossip rows** (see [§2](#2-gossip-mechanism)), where they are the sender’s opinion about *other* nodes and feed failure reports.

### 1.3 Data by message type

`hdr->type` selects the `union clusterMsgData` arm. The fixed header ([§1.2.2](#122-header-fields)) is always present; only `data` (and how `count` / `mflags` / epochs are interpreted) changes.

| § | Type | Id | `data` arm |
|---|------|----|------------|
| [1.3.1](#131-ping--pong--meet-02--dataping) | `PING` / `PONG` / `MEET` | 0–2 | `data.ping` |
| [1.3.2](#132-fail-3--datafail) | `FAIL` | 3 | `data.fail` |
| [1.3.3](#133-publish-4--publishshard-10--datapublish) | `PUBLISH` / `PUBLISHSHARD` | 4 / 10 | `data.publish` |
| [1.3.4](#134-failover_auth_request-5--empty-data) | `FAILOVER_AUTH_REQUEST` | 5 | (empty) |
| [1.3.5](#135-failover_auth_ack-6--empty-data) | `FAILOVER_AUTH_ACK` | 6 | (empty) |
| [1.3.6](#136-update-7--dataupdate) | `UPDATE` | 7 | `data.update` |
| [1.3.7](#137-mfstart-8--empty-data) | `MFSTART` | 8 | (empty) |
| [1.3.8](#138-module-9--datamodule) | `MODULE` | 9 | `data.module` |

#### 1.3.1 `PING` / `PONG` / `MEET` (0–2) — `data.ping`
...
- `hdr->count` = number of gossip entries (see [§2.2 Sampling](#22-sampling)).
...
#### 1.3.3 `PUBLISH` (4) / `PUBLISHSHARD` (10) — `data.publish`
...
#### 1.3.4 `FAILOVER_AUTH_REQUEST` (5) — empty `data`
...
#### 1.3.5 `FAILOVER_AUTH_ACK` (6) — empty `data`
...
### 1.4 Slots and epochs on the wire

Slot ownership and config freshness ride in every gossip **header** (`myslots`, `configEpoch`, `currentEpoch`). When a node learns a **higher** config epoch for a peer’s slots than it currently believes, an `UPDATE` packet’s **body** ([§1.3.6](#136-update-7--dataupdate)) can push that peer’s name, epoch, and full slot bitmap so redirects stay correct after failover or resharding.

### 1.5 Design properties (wire)
...
Gossip sampling and failure quorum are covered in [§2](#2-gossip-mechanism).
...
Wire layouts for those types are in [§1.3](#13-data-by-message-type).
...
Cron opens bus links and schedules pings toward peers that look stale relative to `cluster-node-timeout`. Each successful exchange updates the sender from the peer’s header and merges the peer’s gossip rows into the local `nodes` dictionary.
...
**What each sampled row carries.** A rumor about one third party: id, coarse `ping_sent` / `pong_received` (seconds), IP and ports, and that peer’s `flags` as the **sender** sees them (`MASTER` / `SLAVE` / `PFAIL` / `FAIL` / `NOADDR`, …). Exact field sizes are under [§1.3.1](#131-ping--pong--meet-02--dataping).
...
### 2.4 Failure detection (weak quorum)
...
2. Masters advertise `PFAIL`/`FAIL` in **gossip rows**; peers record **failure reports** from masters. PFAIL bias in [§2.2 Sampling](#22-sampling) accelerates this.
...
This agreement is intentionally weak and time-based: partitions may delay visibility, and without majority no replica promotion is authorized. Automatic failover then uses `FAILOVER_AUTH_REQUEST` / `ACK`; manual failover uses `MFSTART` plus forced ack semantics.

### 2.5 Design properties (gossip)
...
### 3.2 Wire format: `clusterMsg`

Binary framing as designed in §1.2. Concrete header fields:
...
|---------------------|----|----------------|---------|
| `PING` / `PONG` / `MEET` | 0–2 | `clusterMsgDataGossip gossip[count]` (+ optional extensions) | [§1.3.1](#131-ping--pong--meet-02--dataping) |
| `FAIL` | 3 | failed node name (`clusterMsgDataFail`) | [§1.3.2](#132-fail-3--datafail) |
| `PUBLISH` / `PUBLISHSHARD` | 4 / 10 | channel + payload lengths and bytes | [§1.3.3](#133-publish-4--publishshard-10--datapublish) |
| `FAILOVER_AUTH_REQUEST` / `ACK` | 5 / 6 | empty beyond header (vote semantics in header/epochs) | [§1.3.4](#134-failover_auth_request-5--empty-data) / [§1.3.5](#135-failover_auth_ack-6--empty-data) |
| `UPDATE` | 7 | config epoch, node name, slots bitmap | [§1.3.6](#136-update-7--dataupdate) |
| `MFSTART` | 8 | empty beyond header | [§1.3.7](#137-mfstart-8--empty-data) |
| `MODULE` | 9 | module id, type, payload | [§1.3.8](#138-module-9--datamodule) |
...
### 3.4 Receiving: `clusterProcessGossipSection`
...
| Topic | Summary |
|-------|---------|
| Architecture | Planes; protocol (§1.2); data by type §1.3.1–1.3.8; gossip (§2) |
| Gossip mechanism | Exchange, sampling (`N/10` + PFAIL), merge, failure quorum (§2) |
| Implementation | `clusterMsg` structs; `clusterSendPing` / `clusterProcessGossipSection` / `clusterProcessPacket` |
